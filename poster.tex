...
A makefile and source files containing everything
...
\begin{center}
\includegraphics[width=0.5\textwidth]{hyades_uvw.pdf}
\end{center}
...
for order \hl{$10^6$--$10^7$ rows}, on modest hardware
with no special data preparation
(e.g.\ data downloaded from external services, FITS files, even CSV).
For much larger tables, interactive performance will degrade and
...
most tasks scaling as $O(N^1)$ in processing time and $O(N^0)$
in memory for $N$ rows.
...
{\color{grey}
 ...
 \hfill
 {\tt https://github.com/mbtaylor/adass2018-tcgaia \input{sha1}}
}
\end{textblock}
